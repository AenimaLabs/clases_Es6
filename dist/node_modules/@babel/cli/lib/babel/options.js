...
function commander() {
  var data = _interopRequireWildcard(require("commander"), true);
...
function glob() {
  var data = _interopRequireWildcard(require("glob"), true);
  glob = function glob() {
    return data;
  };
  return data;
...
var program = commander()["default"].program;
...
function parseArgv(args) {
  ...
  var errors = [];
  var filenames = program.args.reduce(function (globbed, input) {
    var files = glob().sync(input);
    if (!files.length) files = [input];
    globbed.push.apply(globbed, _toConsumableArray(files));
    return globbed;
  }, []);
